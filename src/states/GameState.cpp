...
{
	if (!this->backTexture.loadFromFile("../res/images/sky.png")) { std::cout << "error load img background!" << std::endl; }
...
{
    this->window = window;
	this->viewSizeX = this->window->getSize().x*0.9;
	this->viewSizeY = this->window->getSize().y*0.9;
    ...
    this->player = new Player(this->window,"../res/images/myhero.png",viewSizeX/2,100);
    this->player->view.reset(sf::FloatRect(0,0, window->getSize().x*0.9, window->getSize().y));
	this->player->view.zoom(0.6);
	this->player->view.setCenter(this->viewSizeX / 2, this->viewSizeY / 2);
