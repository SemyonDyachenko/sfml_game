...
SettingsParsers::SettingsParsers() : m_isChanged(false)
{
...
bool SettingsParsers::loadFromFile(const std::string& filename)
{
	m_data.clear();
	m_filename = filename;
	return read();
...
bool SettingsParsers::saveToFile()
{
	if (m_isChanged)
	{
		m_isChanged = false;
...
bool SettingsParsers::write() const
{
 ...
 */
std::pair<std::string, std::string> SettingsParsers::parseLine(const std::string &line) const
{
...
void SettingsParsers::print() const
{
...
bool SettingsParsers::isChanged() const
{
	return m_isChanged;
}